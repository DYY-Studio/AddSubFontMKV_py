...
# bold_font 列表、italic_font 列表结构
#   [ 字体名称 ]
def fontProgress(fl: list, f_n: dict = {}) -> dict:
    global dupfont, font_family
    global font_n_lower
    #print(fl)
    flL = len(fl)
            ...
            for ii in range(0, len(t['name'].names)):
                ...
                if name.nameID == 1:
                    try:
                        ...
                        familyN = name.toBytes().decode('utf-16-be', errors='ignore')
                        try:
                            if len([i for i in name.toBytes() if i == 0]) > 0:
                                ...
                                familyN = nnames.names[ii].toStr()
                            else: familyN = name.toBytes().decode('utf-16-be', errors='ignore')
                        except:
                            pass
...
def checkAssFont(fontlist: dict, font_name: dict, assfont: dict = {}, onlycheck: bool = False):
    # 从fontlist获取字体名称
    global font_n_lower, font_family
    ...
    if not notfont:
        ...
        for s in asspath:
            if path.splitext(s)[1][1:].lower() not in ['ass', 'ssa']:
                ...
                continue
            else: newasspath.append(assFontChange(multiAss[s][0], newfont_name, s, multiAss[s][1], multiAss[s][2], outdir[0], fn_lines=fn_lines))
    else:
        newasspath = asspath
        newfont_name = {}
    if mux: 
...
os.system('title ASFMKV Python Remake 1.01 ^| (c) 2022 yyfll ^| Apache-2.0')
                    ...
                    if s_subdir:
                        ...
                        for f in os.listdir(cpath):
                            if path.isfile(path.join(cpath, f)):
                                ...
                                if path.splitext(f)[1][1:].lower() in ['ass', 'ssa']:
                                    #print(f, 'pass')
                                    a, fontlist, b, c, d = assAnalyze(path.join(cpath, f), fontlist, onlycheck=True)
                                    assfont, font_name = checkAssFont(fontlist, font_name, assfont, onlycheck=True)
                    fd = path.join(cpath, 'Fonts')
                else:
                    a, fontlist, b, c, d = assAnalyze(cpath, fontlist, onlycheck=True)
                    assfont, font_name = checkAssFont(fontlist, font_name, assfont, onlycheck=True)
...
if __name__=="__main__":
    while 1:
        work = 0
        print('''ASFMKV Python Remake 1.01 | (c) 2022 yyfll{0}
