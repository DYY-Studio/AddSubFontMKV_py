...
def assAnalyze(asspath: str, fontlist: dict = {}, onlycheck: bool = False):
    ...
    for i in range(styleline + 2, asslen):
        ...
        if len(ssRecover) > 0:
            if ssRecover.get(font_key) is not None:
                fullass[i] = fullass[i].replace(font_key, ssRecover[font_key])
    ...
    for i in range(eventline + 2, asslen):
        ...
        if len(itl) > 0:
            ...
            # for nsti in range(len(newstrl), 0, -1):
            #     newstrl
            fn = ''
            for sp in splitpos:
                            ...
                            if 'fn' in sf.lower():
                                ...
                                if len(ssRecover) > 0:
                                    if ssRecover.get(fn) is not None:
                                        fullass[i] = fullass[i].replace(fn, ssRecover[fn])
                                        fn = ssRecover[fn]
...
def assFontChange(fullass: list, newfont_name: dict, asspath: str, styleline: int, infoline: int,
    ...
    fullass.insert(infoline + 1, '; ASFMKV_py 1.02-Pre4\n')
...
os.system('title ASFMKV Python Remake 1.02-Pre4 ^| (c) 2022 yyfll ^| Apache-2.0')
                        ...
                        for r,ds,fs in os.walk(cpath):
                            for f in fs:
                                if path.splitext(f)[1][1:].lower() in ['ass', 'ssa']:
                                    ...
                                    del a,b,c,d,e
                                    assfont2, font_info = checkAssFont(fontlist, font_info, onlycheck=True)
                                    assfont = updateAssFont(assfont, assfont2)
                                    del assfont2
                    else:
...
if __name__=="__main__":
    while 1:
        work = 0
        print('''ASFMKV Python Remake 1.02-Pre4 | (c) 2022 yyfll{0}
