    ...
    for s in keys:
        ...
        if not cok:
            ...
            if not onlycheck:
                ...
                # 否则直接添加空
                assfont['?'.join([ns, ns])] = ['', sp[0], ns, [checkf]]
        ...
        if cok:
            ...
            if directout < 3:
                ...
                if assfont.get(dict_key) is None:
                    assfont[dict_key] = [fontlist[s], sp[0], ns, [checkf]]
                else:
                    ...
                    for newfname in newfnamep:
                        ...
                        assfont[dict_key] = [oldstr, newfname, tfname, list(set(assfont[dict_key][3] + [checkf]))]
                        fontlist[s] = oldstr
                    # else:
...
def assFontSubset(assfont: dict, fontdir: str, font_info: list):
    """
字体子集化
  ...
  newfont_name: { 原字体名 : [ 新字体绝对路径, 新字体名 ] }
    ...
    for k in assfont.keys():
        ...
        if gfs_uni.get('textInput', False):
            print('\n\033[1;31m[WARNING] 字符缺失检查失败，该字体将无法正常子集化\033[0m')
        else:
        ...
        try:
            ...
            if len(sp) > 1:
                for n in sp:
                    newfont_name[n] = [subfontpath, subfontcrc]
            else:
        ...
        if path.exists(subfontpath):
            ...
            if path.exists(crcnewf):
                ...
                if len(sp) > 1:
                    for n in sp:
                        newfont_name[n] = [crcnewf, subfontcrc]
                else:
                    newfont_name[s[3]] = [crcnewf, subfontcrc]
    print('')
    # print(newfont_name)
...
def assFontChange(fullass: list, newfont_name: dict, asspath: str, styleline: int, infoline: int,
                  font_pos: int, text_pos: int, outdir: str = '', ncover: bool = False, fn_lines: list = []) -> str:
    """
更改ASS样式对应的字体
...
需要以下输入
  ...
  text_pos: Text内容在 Dialogue 中位于第几个逗号之后
...
将会返回以下
  mkvmr: mkvmerge命令行的返回值    
    ...
    if len(fontpaths) > 0:
        for s in fontpaths:
            mkvargs.extend(['--attachment-name', path.basename(s[0])])
    ...
    if not notfont:
        ...
        for i in range(0, len(asspath)):
            ...
            if path.splitext(s)[1][1:].lower() not in ['ass', 'ssa']:
                ...
                fullass, fontlist, styleline, font_pos, text_pos, fn_lines, infoline = assAnalyze(s, fontlist)
                multiAss[s] = [fullass, styleline, font_pos, infoline, text_pos]
        ...
        for s in asspath:
            if path.splitext(s)[1][1:].lower() not in ['ass', 'ssa']:
                ...
                newasspath.append(
                    assFontChange(multiAss[s][0], newfont_name, s, multiAss[s][1], multiAss[s][3], multiAss[s][2], multiAss[s][4],
...
def cListAssFont(font_info):
    global resultw, s_subdir, copyfont, fontload, exact_lost, char_lost
    font_name = font_info[0]
    leave = True
    while leave:
        fontlist = {}
                    ...
                    if s_subdir:
                        for r, ds, fs in os.walk(cpath):
                            for f in fs:
                                if path.splitext(f)[1][1:].lower() in ['ass', 'ssa']:
                                    a, fontlist, b, c, d, g, h = assAnalyze(path.join(r, f), fontlist, onlycheck=True)
                                    del a, b, c, d, g, h
                        ...
                        for f in os.listdir(cpath):
                            if path.isfile(path.join(cpath, f)):
                                ...
                                if path.splitext(f)[1][1:].lower() in ['ass', 'ssa']:
                                    # print(f, 'pass')
                                    a, fontlist, b, c, d, g, h = assAnalyze(path.join(cpath, f), fontlist, onlycheck=True)
                                    del a, b, c, d, g, h
                                    assfont2, font_info2 = checkAssFont(fontlist, font_info2, onlycheck=True, checkf=f)
                                    assfont = updateAssFont(assfont, assfont2)
                                    del assfont2
...
'''.format(v_subdir, s_subdir, rmAssIn, rmAttach, mkvout, assout,
           fontout, matchStrict, notfont, char_compatible, errorStop, fontload, ignoreLost))
...
def cLicense():
    cls()
    print('''AddSubFontMKV Python Remake Preview 13
Apache-2.0 License
...
'''.format(s_subdir)).strip(' ')) == 0:
    ...
    if path.isdir(sPath):
        ...
        if s_subdir:
            for r,d,fs in os.walk(sPath):
                for f in fs:
                    if path.splitext(f)[1][1:].lower() in ['ass','ssa']:
                        try:
                            TCDetect_Trans(path.join(r,f), path.join(outPath, '.utf8'.join(path.splitext(f))))
                        except Exception as e:
                            print('\033[31;1m{}\033[0m'.format(e))
        else:
            for f in os.listdir(sPath):
                ...
                if path.isfile(f):
                    if path.splitext(f)[1][1:].lower() in ['ass','ssa']:
                        try:
                            TCDetect_Trans(f, path.join(outPath, '.utf8'.join(path.splitext(path.basename(f)))))
                        except Exception as e:
                            print('\033[31;1m{}\033[0m'.format(e))
    else:
        outPath = path.join(path.dirname(sPath), 'UTF8-Subtitles')
        try:
...
# def OFDetect_Trans(f: str, of: str):
#     if path.splitext(f)[1][1:].lower() in ['ttf', 'otf']:
#         try:
#             ts = [ttLib.TTFont(f)]
#         except:
#             ts = ttLib.TTCollection(f)
#     else:
#         try:
#             ts = ttLib.TTCollection(f)
#         except:
...
#                 ts.close()
#             else: t.close()
#             print('跳过不支持字体 \033[33;1m\"{}\"\033[0m'.format(path.basename(f)))
#             return None
#         # 如果第一步的Try中获取到了Unicode CMAP，则直接处理 Unicode CMAP
#         # 处理非常简单: 将字符转换为 Unicode 编码对应的十进制数 (ord)，然后在词典里查找即可
#         # 如果词典里没有，就是没有
...
#                 ts.close()
#             else: t.close()
#             print('跳过标准字体 \033[33;1m\"{}\"\033[0m'.format(path.basename(f)))
#             return None
#         else:
#             # 找 ANSI CMAP
#             if len(t['cmap'].tables) > 0:
#                 tcoding = ''
#                 for tt in t['cmap'].tables:
#                     if len(tt.cmap) > len(cmap):
#                         cmap = tt.cmap
#                         tcoding = tt.getEncoding()
#                         use_table = tt
#                 # 如果这也找不到，那就退
#                 if len(cmap) == 0:
#                     t.close()
...
#                 # fontTools默认返回GB2312，但是GB2312范围太小，如"·"是无法解码的，这里替换为GBK来解决这一问题
#                 if tcoding.lower() in ['gb2312', 'x_mac_simp_chinese_ttx']: tcoding = 'gbk'
#                 print('\n\033[1;33m\"{0}\"\033[0m >>> \033[1;33m\"Unicode\"\033[0m'.format(tcoding))
#                 for i in cmap.keys():
#                     if i < 256:
#                         gfs_uni[i] = i
#                     else:
#                         try:
#                             c = i.to_bytes(2, 'big').decode(tcoding)
#                         except:
#                             continue
#                         # 如果是超出 ASCII 字符范围的，则将其用对应编码encode，然后将得到的 bytes 转换为十六进制，再转为十进制
#                         dec = int(''.join([hex(h).lstrip('0x') for h in c.encode(tcoding)]), 16)
#                         if dec >= 0:
#                             # 创建一个词典 gfs_uni，里面是 本地文本编码十进制 到 Unicode十进制 的对应关系
#                             # 以便修复CMAP使用
#                             gfs_uni[dec] = ord(c)
...
# def cOutOfDateFontsTrans():
#     cls()
#     print('Out-of-Date Fonts Transform Batch')
#     fPath = ''
#     f_subdir = False
#     loop = 0
#     while not (path.isdir(fPath) or (path.isfile(fPath) and path.splitext(fPath)[1][1:].lower() in ['otf', 'ttf', 'ttc', 'otc'])):
#         loop += 1
#         if loop > 4: return None
#         fPath = input('字体路径或字体所在目录路径:').strip('\"').strip('\'').strip(' ')
#     loop = 0
#     if path.isdir(fPath):
#         while len(input('''
# 搜索子目录: {}
...
# \033[33;1m输入任意键(除空格)并回车\033[0m进入到下一步
# \033[33;1m不输入任何值\033[0m切换是否搜索子目录
# '''.format(f_subdir)).strip(' ')) == 0:
#             loop += 1
#             if loop > 6: return None
#             f_subdir = not f_subdir
#     outPath = ''
#     fn = 0
#     if path.isdir(fPath):
#         outPath = path.join(fPath, 'Newformat_Fonts')
#     else:
#         outPath = path.join(path.dirname(fPath), 'Newformat_Fonts')
#     if not path.exists(outPath): 
#         try:
#             os.mkdir(outPath)
#         except Exception as e:
...
#                 for f in fs:
#                     if path.splitext(f)[1][1:].lower() in ['otf','otc','ttc','ttf']:
#                         try:
#                             OFDetect_Trans(path.join(r,f), path.join(outPath, '.newformat'.join(path.splitext(f))))
#                         except Exception as e:
#                             print('\033[31;1m{}\033[0m'.format(e))
#         else:
...
#                     if path.splitext(f)[1][1:].lower() in ['otf','otc','ttc','ttf']:
#                         try:
#                             OFDetect_Trans(f, path.join(outPath, '.newformat'.join(path.splitext(path.basename(f)))))
#                         except Exception as e:
#                             print('\033[31;1m{}\033[0m'.format(e))
#     else:
#         # try:
#         OFDetect_Trans(fPath, path.join(outPath, '.newformat'.join(path.splitext(path.basename(fPath)))))
#         # except Exception as e:
#         #     print('\033[31;1m{}\033[0m'.format(e))
#     if not len(os.listdir(outPath)):
#         try:
...
def loadMain(reload: bool = False):
    ...
    os.system('title ASFMKV Python Remake Pre13 ^| (c) 2022 yyfll ^| Apache-2.0')
    if not reload:
        if not o_fontload:
    ...
    if o_fontload:
        ...
        fltip = '包含乱码的名称'
    print('''ASFMKV Python Remake Pre13 | (c) 2022 yyfll{0}
...
[L] 直接退出'''.format(mkvmv, len(dupfont.keys()), len(font_info[0].keys()), len(font_info[3]), fltip))
    print('')
    work = os.system('choice /M 请选择: /C ABCSWDLRMU')
    if work == 1:
