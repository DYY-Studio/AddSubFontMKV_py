...
def assAnalyze(asspath: str, fontlist: dict = {}, onlycheck: bool = False):
    ...
    for s in fontlist.keys():
        if len(fontlist[s]) == 0:
            ...
            if not onlycheck: print('\033[1m\"{0}\"\033[0m: 字符数[\033[1;33m{1}\033[0m]'.format(s.split('?')[0], len(fontlist[s])))
    # 删去 删去列表 中的字体
    if len(fl_popkey) > 0:
        for s in fl_popkey:
            fontlist.pop(s)
...
def checkAssFont(fontlist: dict, font_info: list, fn_lines: list = [], assfont: dict = {}, onlycheck: bool = False):
    # 从fontlist获取字体名称
...
def assFontChange(fullass: list, newfont_name: dict, asspath: str, styleline: int, infoline: int,
    ...
    for k in newfont_name.keys():
        fullass.insert(infoline + 1, '; Font Subset: {0} - {1}\n'.format(k, newfont_name[k][1]))
    fullass.insert(infoline + 1, '; ASFMKV_py 1.02-Pre3\n')
...
os.system('title ASFMKV Python Remake 1.02-Pre3 ^| (c) 2022 yyfll ^| Apache-2.0')
...
[Z] 子集化失败中断: \033[1;33m{10}\033[0m
'''.format(v_subdir, s_subdir, rmAssIn, rmAttach, mkvout, assout, 
fontout, matchStrict, notfont, warningStop, errorStop))
        work = 0
        work = os.system('choice /M 请输入 /C AB1234567890XZL')
...
'''.format(rmAssIn, rmAttach, notfont, mkvout, v_subdir, s_subdir, matchStrict, errorStop, warningStop))
            cpath = ''
            directout = False
            subonly = False
            subonlyp = ''
...
[L] 直接退出'''.format(mkvmv, len(dupfont.keys()), len(font_info[0].keys()), len(font_info[3])))
        print('')
        work = os.system('choice /M 请选择: /C ABCSWDL')
        if work == 1:
