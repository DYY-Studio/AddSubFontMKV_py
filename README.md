...
* Bold、Italic与Regular分离的字体不能正确处理（正在处理）<br>*拓展：增加`\i` `\b`粗斜体标签分析，变更字体对应表格式*<br>*拓展：字体家族索引表*
* ASS/SSA 字体扫描结果不会重置（可能是开发版的问题，正在处理）
* ~~当`\fn`不是第一个标签时，检测不到`\fn`指定的字体~~（已修复）
* ~~大ASS/SSA文件读取过慢~~（已优化）

| 当前测试字幕 | 字幕组 | 测试内容 |
| --- | --- | --- |
| 僕だけがいない街 | 诸神 简繁日 | 单行多`\b` `\i`标签，单行多`\fn`标签
