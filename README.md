...
## 关于重大BUG
在对 **动漫国《ぼっち・ざ・ろっく！》** 字幕进行分析时发现了严重的BUG<br>该BUG是由 ASS/SSA 字幕特效标签分析部分考虑不周导致的

有以下问题:
* 无法处理`\r`特效重置标签
* 无法处理单特效块内的多个同类特效标签
* 特效标签定位由左搜索而非由右搜索，可能导致标签是否启用判断失误

`\r`标签的支持已在 Preview 14 中添加，但剩余问题仍亟待解决<br>
不敢发布正式版就是担心今天这种情况，对造成的各种不便深表歉意<br>
该更新之后会追加到 PyQt Beta2 Fix5
