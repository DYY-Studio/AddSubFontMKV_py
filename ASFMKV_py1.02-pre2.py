...
def assAnalyze(asspath: str, fontlist: dict = {}, onlycheck: bool = False):
    ...
    for i in range(0, len(style_format)):
        ...
        if s == 'name':
            ...
            italic_pos = i
        if style_pos != 0 and font_pos != 0 and bold_pos != 0 and italic_pos != 0:
            break
    ...
    for i in range(eventline + 2, asslen):
        ...
        if re.search(r'\{.*?(?:\\b[7-9]00|\\b1|\\i1).*?\}', eventftext) is not None:
            lastfind = 0
            for st in re.findall(r'\{.*?\}', eventftext):
        ...
        if it is not None:
            ...
            while it is not None:
                ...
                if it[0] > 0:
                    ...
                    newstrl = []
                    for sp in splitpos:
                        if fnpos in range(sp[0], sp[1]):
                            if fnpos + len(cuttext) not in range(sp[0], sp[1]):
                                newstrl.append([eventftext2[fnpos:sp[1]], str(sp[2]), str(sp[3])])
                            else:
                                newstrl.append([cuttext, str(sp[2]), str(sp[3])])
                        elif fnpos + len(cuttext) in range(sp[0], sp[1]):
                            newstrl.append([eventftext2[sp[0]:(fnpos + len(cuttext))], str(sp[2]), str(sp[3])])
                ...
                fnpos = eventftext2.find(eventftext)
                newstrl = []
                for sp in splitpos:
                    if fnpos in range(sp[0], sp[1]):
                        if fnpos + len(eventftext) not in range(sp[0], sp[1]):
                            newstrl.append([eventftext2[fnpos:sp[1]], str(sp[2]), str(sp[3])])
                        else:
                            newstrl.append([eventftext, str(sp[2]), str(sp[3])])
                    elif fnpos + len(eventftext) in range(sp[0], sp[1]):
                        newstrl.append([eventftext2[sp[0]:(fnpos + len(eventftext))], str(sp[2]), str(sp[3])])
                for fi in range(0, len(newstrl)):
                    fs = newstrl[fi]
                    fss = re.sub(effectDel, '', fs[0])
                    if len(fn) > 0:
...
def fnReadCorrect(ttfont: ttLib.ttFont, index: int, fontpath: str) -> str:
    ...
    try:
        ...
        if len([i for i in name.toBytes() if i == 0]) > 0:
            ...
            print('\n\033[1;33m已修正字体\"{0}\"名称读取 >> \"{1}\"\033[0m'.format(path.basename(fontpath), namestr))
            #os.system('pause')
        else: namestr = name.toBytes().decode('utf-16-be', errors='ignore')
        # 如果没有 \x00 字符，使用 utf-16-be 强行解码；如果有，尝试解码；如果解码失败，使用 utf-16-be 强行解码
    except:
        print('\n\033[1;33m尝试修正字体\"{0}\"名称读取 >> \"{1}\"\033[0m'.format(path.basename(fontpath), namestr))
    return namestr
...
# font_info 列表结构
#   [ font_name, font_n_lower, font_family ]
# font_name 词典结构
        ...
        for ti in range(0, len(tc)):
            ...
            for ii in range(0, indexs):
                ...
                if name.nameID == 1:
                    ...
                    font_family.setdefault(familyN.strip(' '), {})
                # 若 nameID 为 2，读取 NameRecord 的字体样式
                if name.nameID == 2 and not donotStyle:
                    try:
                        fstyle = name.toStr()
                    except:
                ...
                if name.nameID == 4:
                    ...
                    if namestr.strip(' ') not in namestrl: namestrl.append(namestr.strip(' '))
            ...
            for namestr in namestrl:
                ...
                # if f_n.get(namestr) is not None:
                    # if isCustom and not isBold and not isItalic and fstyle.lower().strip(' ') != 'regular' and namestr.lower().find(fstyle.lower()) < 0:
                    #     if f_n.get(namestr + ' ' + fstyle) is not None:
                    #         pass
                    #     else:
                    #         namestr = namestr + ' ' + fstyle
                    # elif isBold and isItalic and namestr.lower().find('bold') < 0 and namestr.lower().find('italic') < 0:
                    #     if f_n.get(namestr + ' Bold Italic') is not None or f_n.get(namestr + ' Italic Bold') is not None:
                    #         pass
                    #     else:
                    #         namestr = namestr + ' Bold Italic'
                    # elif isBold and namestr.lower().find('bold') < 0:
                    #     if f_n.get(namestr + ' Bold') is not None:
                    #         pass
                    #     else:
                    #         namestr = namestr + ' Bold'
                    # elif isItalic and namestr.lower().find('italic') < 0:
                    #     if f_n.get(namestr + ' Italic') is not None:
                    #         pass
                    #     else:
                    #         namestr = namestr + ' Italic'
...
def checkAssFont(fontlist: dict, font_info: list, assfont: dict = {}, onlycheck: bool = False):
    # 从fontlist获取字体名称
    ...
    for s in keys:
        ...
        isitalic = int(sp[1])
        ns = sp[0]
        if font_family.get(sp[0]) is not None:
            if font_family[sp[0]].get((isitalic, isbold)) is not None:
                ns = font_family[sp[0]][(isitalic, isbold)][0]
            elif font_family[sp[0]].get((0, isbold)) is not None:
                ns = font_family[sp[0]][(0, isbold)][0]
            elif font_family[sp[0]].get((isitalic, 0)) is not None:
                ns = font_family[sp[0]][(isitalic, 0)][0]
            elif font_family[sp[0]].get((0, 0)) is not None:
                ns = font_family[sp[0]][(0, 0)][0]
        cok = False
        # 在全局字体名称词典中寻找字体名称
        ss = ns
...
def assFontChange(fullass: list, newfont_name: dict, asspath: str, styleline: int, infoline: int,
    ...
    for i in range(styleline + 2, len(fullass)):
        if len(fullass[i].split(':')) < 2: 
            if i + 31 < len(fullass) - i:
                if re.search(style_read, '\n'.join(fullass[i + 1:30])) is None: 
                    break
                else:
                    continue
            else:
    ...
    if not notfont:
        ...
        for i in range(0, len(asspath)):
            ...
            if path.splitext(s)[1][1:].lower() not in ['ass', 'ssa']:
                ...
                assfont, font_info = checkAssFont(fontlist, font_info, assfont)
                if assfont is None:
                    return None, None, -2
...
os.system('title ASFMKV Python Remake 1.02-Pre2 ^| (c) 2022 yyfll ^| Apache-2.0')
                        ...
                        for r,ds,fs in os.walk(cpath):
                            for f in fs:
                                if path.splitext(f)[1][1:].lower() in ['ass', 'ssa']:
                                    ...
                                    del a,b,c,d,e
                                    assfont, font_info = checkAssFont(fontlist, font_info, assfont, onlycheck=True)
                    else:
...
[Z] 旧格式字体中断: \033[1;33m{9}\033[0m
'''.format(v_subdir, s_subdir, rmAssIn, rmAttach, mkvout, assout, 
fontout, matchStrict, notfont, warningStop))
        work = 0
        work = os.system('choice /M 请输入 /C AB1234567890ZL')
...
'''.format(v_subdir, s_subdir, matchStrict, assout, fontout, warningStop))
            else: print('''子集化字体并封装
...
旧格式字体中断: \033[1;33m{5}\033[0m
'''.format(rmAssIn, rmAttach, notfont, mkvout, v_subdir, s_subdir, matchStrict, warningStop))
            cpath = ''
            directout = False
            subonly = False
            subonlyp = ''
                ...
                if not medias is None:
                    if len(medias) > 0:
                        ...
                        for k in media_ass.keys():
                            ...
                            newasspaths, newfont_name, mkvr = main(font_info, media_ass[k], 
                    ...
                    newasspaths, newfont_name, mkvr = main(font_info, [subonlyp], 
...
def cLicense():
    cls()
    print('''AddSubFontMKV Python Remake 1.01
Apache-2.0 License
...
if __name__=="__main__":
    while 1:
        work = 0
        print('''ASFMKV Python Remake 1.02-Pre2 | (c) 2022 yyfll{0}
...
[L] 直接退出'''.format(mkvmv, len(dupfont.keys()), len(font_info[0].keys()), len(font_info[3])))
        print('')
        work = os.system('choice /M 请选择: /C ABCSWRDL')
        if work == 1:
