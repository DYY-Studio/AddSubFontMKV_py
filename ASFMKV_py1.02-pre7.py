...
def assFontChange(fullass: list, newfont_name: dict, asspath: str, styleline: int, infoline: int,
    ...
    fullass.insert(infoline + 1, '; ASFMKV_py 1.02-Pre7\n')
...
'''.format(rmAssIn, rmAttach, notfont, mkvout, v_subdir, s_subdir, matchStrict, errorStop, warningStop))
            ...
            if not directout:
                ...
                if not medias is None:
                    ...
                    newasspaths, newfont_name, mkvr = main(font_info2, [subonlyp], 
...
def cLicense():
    cls()
    print('''AddSubFontMKV Python Remake 1.02 Preview7
Apache-2.0 License
...
def loadMain(reload: bool = False):
    ...
    os.system('title ASFMKV Python Remake 1.02-Pre7 ^| (c) 2022 yyfll ^| Apache-2.0')
    if not reload:
    ...
    cls()
    print('''ASFMKV Python Remake 1.02-Pre7 | (c) 2022 yyfll{0}
...
[L] 直接退出'''.format(mkvmv, len(dupfont.keys()), len(font_info[0].keys()), len(font_info[3])))
    print('')
    work = os.system('choice /M 请选择: /C ABCSWDLR')
    if work == 1:
