...
def fnReadCheck(ttFont: ttLib.TTFont, index: int, fontpath: str):
    ...
    try:
        try:
            ...
            s = fnReadCorrect(ttFont, index, fontpath)
            isWarningFont = True
        s2 = s
        ...
        if re.search(charEx, s):
            ...
            if not re.search(charEx, s2):
                if len(c) > 0:
                    ...
                    return s2, isWarningFont
                else:
                    return s, isWarningFont
            else:
            ...
            #             c = dc['encoding']
            #             break
        else: return s, isWarningFont
    except Exception as e:
        print(e)
        return fnReadCorrect(ttFont, index, fontpath), True
    ...
    for si in range(0, flL):
        ...
        for ti in range(0, len(tc)):
            ...
            for ii in range(0, indexs):
                ...
                if name.nameID == 1:
                    familyN, isWarningFont = fnReadCheck(t, ii, s)
        ...
        for f in [path.join(cpath, s) for s in os.listdir(cpath)
            ...
            if len(medias) > 0:
                for li in medias:
                    ...
                    sext = path.splitext(f)
                    if (li[0] in f and not matchStrict) or (li[0] == path.basename(f)[:len(li[0])] and matchStrict):
                        if path.dirname(li[1]) == path.dirname(f):
                            if sext[1][:1].lower() == 'idx':
                                if not path.exists(sext[1] + '.sub'):
                                    continue
                            if media_ass.get(li[1]) is None:
...
def cListAssFont(font_info):
    global resultw, s_subdir, copyfont, fontload, exact_lost
    font_name = font_info[0]
    leave = True
    while leave:
        fontlist = {}
...
'''.format(resultw, copyfont, s_subdir, fontload, exact_lost))
        work = os.system('choice /M 请输入 /C AB12345L')
        if work == 1:
            cls()
            ...
            if not directout:
                if path.isdir(cpath):
                    ...
                    if s_subdir:
                        for r, ds, fs in os.walk(cpath):
                            for f in fs:
                                if path.splitext(f)[1][1:].lower() in ['ass', 'ssa']:
                                    a, fontlist, b, c, d, g = assAnalyze(path.join(r, f), fontlist, onlycheck=True)
                                    del a, b, c, d, g
                        ...
                        for f in os.listdir(cpath):
                            if path.isfile(path.join(cpath, f)):
                                ...
                                if path.splitext(f)[1][1:].lower() in ['ass', 'ssa']:
                                    # print(f, 'pass')
                                    a, fontlist, b, c, d, g = assAnalyze(path.join(cpath, f), fontlist, onlycheck=True)
                                    del a, b, c, d, g
                                    assfont2, font_info2 = checkAssFont(fontlist, font_info2, onlycheck=True, checkf=f)
                                    assfont = updateAssFont(assfont, assfont2)
                                    del assfont2
                    ...
                    if fontload:
                        ...
                        font_info2 = font_info
                    a, fontlist, b, c, d, g = assAnalyze(cpath, fontlist, onlycheck=True)
                    del a, b, c, d, g
                    assfont2, font_info2 = checkAssFont(fontlist, font_info2, onlycheck=True, checkf=f)
                    assfont = updateAssFont(assfont, assfont2)
                    del assfont2
                    ...
                    for s in assfont.keys():
                        ...
                        if not ssp[0] == ssp[1]:
                            ...
                            if errshow:
                                ...
                                print(
                                    '\033[1;32m[{3}]\033[0m \033[1;36m{0}\033[0m \033[1m<{1}>\033[1;32m{2}\033[0m'
                                    .format(assfont[s][2], path.basename(fn), ann, str(len(assfont[s][0])).rjust(maxnum)))
                            # print(assfont[s][0])
                        else:
                            if resultw:
...
def cLicense():
    cls()
    print('''AddSubFontMKV Python Remake 1.02 Preview11
Apache-2.0 License
...
def loadMain(reload: bool = False):
    ...
    os.system('title ASFMKV Python Remake 1.02-Pre11 ^| (c) 2022 yyfll ^| Apache-2.0')
    if not reload:
        if not o_fontload:
    ...
    if o_fontload:
        ...
        fltip = '包含乱码的名称'
    print('''ASFMKV Python Remake 1.02-Pre11 | (c) 2022 yyfll{0}
